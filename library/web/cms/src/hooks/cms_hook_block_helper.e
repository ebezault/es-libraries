...
feature {NONE} -- Factory / None

	none_template_block (a_module: CMS_MODULE; a_block_id: READABLE_STRING_8; a_cms_api: CMS_API): detachable CMS_NONE_TEMPLATE_BLOCK
			-- Mustache content block for `a_block_id' in the context of `a_module' and `a_cms_api'.
		local
			res: PATH
			p: detachable PATH
		do
...
feature {NONE} -- Factory / Mustache		

	mustache_template_block (a_module: CMS_MODULE; a_block_id: READABLE_STRING_8; a_cms_api: CMS_API): detachable CMS_MUSTACHE_TEMPLATE_BLOCK
			-- Mustache content block for `a_block_id' in the context of `a_module' and `a_cms_api'.
		local
			res: PATH
			p: detachable PATH
	...
	mustache_template_block_with_values (a_module: CMS_MODULE; a_block_id: READABLE_STRING_8; a_cms_api: CMS_API; a_values: STRING_TABLE [ANY]): like mustache_template_block
			-- Mustache content block for `a_block_id' in the context of `a_module' and `a_cms_api',
			-- With additional `a_values'.
		do
			Result := mustache_template_block (a_module, a_block_id, a_cms_api)
			if Result /= Void then
				across
					a_values as v
				loop
